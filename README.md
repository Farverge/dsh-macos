...
> [!NOTE]
> **版本备注（2026-08-29 核实）**：上游开发线已合入「launch token + 签名 Cookie」的浏览器认证链（决策记录 2026-08-24，`browser-auth.ts`：根 URL 一次性 token 换取 HttpOnly 签名 Cookie，RPC 派发前 401 拦截，静态资源保持公开）。本应用当前锁定的 `dsh@0.1.1-rc.2` **不含**该机制（实测根页面与健康接口仍为无认证 200），故上述警示对当前发布版依然准确；待上游版本经我们评估后升级，认证链将随之生效，届时本节同步改写。
