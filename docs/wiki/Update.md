...
1. **联网查询** `@deepseek-ai/dsh@latest`（npm view，纯查询，不停服、不下载）
   ...
   > 版本通道说明：更新模块默认只跟 npm 的 `latest` 稳定线。官方预发布走 npm `alpha` dist-tag（如 `0.1.2-alpha.2`，2026-08-30 出现；更早的 alpha.1 曾只打 GitHub tag）——是否在更新检查里提示预发布，见后续版本的功能开关；手动尝鲜：`npx @deepseek-ai/dsh@alpha`，风险自担。
2. **无新版** → 提示"已是最新版本"，结束
